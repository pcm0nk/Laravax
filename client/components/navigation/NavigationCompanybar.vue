  ...
  <div>
      ...
      >
        ...
        <template v-for="(item,index) in menuarray" :key="index" >
          <v-list-item  class="body-1 "  :active="item.active"  @click="runsecondnav(index,item.id)">
     ...
     @mouseleave="secondnav = false"
      :model-value="secondnav" >
        <v-list v-if="secondnav" v-model:opened="open" open-strategy="multiple">
      <v-list-group  v-for="(sitem,index) in submenuarray" :value="sitem.value" :key="sitem.value">
        ...
        <v-list-item v-for="(mitem,index) in sitem.menuoption" :key="mitem.opid" @click="secondnav = false" :to="mitem.to">
